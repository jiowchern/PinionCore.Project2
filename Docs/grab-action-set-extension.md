...
```
玩家出 GrabStart(ActionConfig.HitEffect=Grab 帶 HitSegments)
  → HitResolver 掃描命中 → GrabResolver.EnqueueGrab(掃描中不動位置)
  → GrabResolver.Tick(同幀、掃描後)驗證並建立 pair:
      grabber ForceTransition → GrabIdleA;victim → GrabIdleB(面向 grabber)
  → 配對存續期間:
      - grabber 的 MoveInfo 加錨點偏移轉發給 victim(負 Speed = 面向 grabber 倒退走)
      - grabber 節點轉移鏡射到 victim(IdleA→IdleB、WalkA→WalkB)
      - Atk1A 不鏡射:HitSegments 命中 victim → Damage 路由 → 轉移圖 Damage=GrabAtk1B
      - ThrowA:起手即解體,victim 自驅 ThrowB 的烘焙飛行
      - victim 出 GrabBreakB(白名單唯一入口)→ 解體,grabber 進 GrabBreakA 後搖
      - grabber 離開 grab 家族(如被第三方打進 BattleDamage)→ 自動解體
```
...
|------|--------------------|------|--------|
| Grab2Start  | 空(起手不可動) | BattleIdle | BattleDamage |
| Grab2IdleA  | Grab2WalkA / Grab2Atk1A / Grab2ThrowA | Grab2IdleA | BattleDamage |
| Grab2WalkA  | 自身(=重定向)/ Grab2IdleA / Grab2Atk1A / Grab2ThrowA | Grab2IdleA | BattleDamage |
| Grab2Atk1A  | 空 | Grab2IdleA | BattleDamage |
| Grab2ThrowA | 空 | BattleIdle | BattleDamage |
| Grab2BreakA | 空 | BattleIdle | BattleDamage |
...
| Grab2ThrowB | 空 | BattleIdle | BattleDamage(配對已解除) |
| Grab2BreakB | 空 | BattleIdle | BattleDamage |

另外把 **Grab2Start 加進出招入口**:`battleIdle.Playables` 與 `battleWalk.Playables`
(第一套的 GrabStart 就在這兩處)。

要點:
- 「A 側被第三方打會放開 B」不用寫任何規則——A 側 Damage 指到 BattleDamage,
  離開家族即觸發 GrabResolver 自動解體。
- B 側 Damage 指到 Grab2Atk1B 是「被抓中挨打(含抓取者補打)仍被抓」的關鍵,別指 BattleDamage。
...
1. `AddressableAssets/Icons/` 加 icon prefab(照 `GrabStart.prefab` 複製改圖,設 Addressable)。
