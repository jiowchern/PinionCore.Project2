...
namespace PinionCore.Project2.Tests
{
    ...
    /// GrabStart 用大半徑命中圈保證同點必中;配對後被抓者吸附錨點、白名單鎖死,
    ...
    public class GrabTests
    {
        ...
        // GrabStart 命中圈半徑 1.5(同點必中)、BattleAttack 半徑 0.3(只打得到同點目標)
        static ActionConfig[] _BuildRoster()
        {
            var grabStart = _Config(ActionType.GrabStart, loop: false, duration: 0.5f);
            grabStart.HitEffect = HitEffectType.Grab;
            grabStart.HitSegments = new[] { _Hit(startTime: 0.05f, duration: 0.4f, radius: 1.5f) };

            var grabAtk1A = _Config(ActionType.GrabAtk1A, loop: false, duration: 0.5f);
            grabAtk1A.HitSegments = new[] { _Hit(startTime: 0.05f, duration: 0.4f, radius: 1.5f) };

            var battleAttack = _Config(ActionType.BattleAttack, loop: false, duration: 0.5f);
            battleAttack.HitSegments = new[] { _Hit(startTime: 0.05f, duration: 0.3f, radius: 0.3f) };

            var grabWalkA = _Config(ActionType.GrabWalkA, loop: true, duration: 0.6f, redirectable: true,
            ...
            return new[]
            {
                ...
                battleAttack,
                grabStart,
                _Config(ActionType.GrabIdleA, loop: true, duration: 0.6f),
                _Config(ActionType.GrabIdleB, loop: true, duration: 0.6f),
                grabWalkA,
                _Config(ActionType.GrabWalkB, loop: true, duration: 0.6f),
                grabAtk1A,
                _Config(ActionType.GrabAtk1B, loop: false, duration: 0.4f),
                _Config(ActionType.GrabThrowA, loop: false, duration: 0.8f),
                _Config(ActionType.GrabThrowB, loop: false, duration: 0.8f, offset: new Vector2(0f, -2.4f)),  // 沿自身背後飛 2.4m
                _Config(ActionType.GrabBreakA, loop: false, duration: 0.5f),
                _Config(ActionType.GrabBreakB, loop: false, duration: 0.5f),
            };
        }
        ...
        // 進場兩人(同點)→ 雙方進 Battle → A 出 GrabStart → 等配對成立(A=GrabIdleA、B=GrabIdleB)
        IEnumerator _EstablishGrab(PinionCore.Project2.Worlds.PlayerController grabber, PinionCore.Project2.Worlds.PlayerController victim)
        {
            ...
            Assert.IsTrue(_Play(grabber, ActionType.BattleIdle), "抓取者進戰鬥應被接受");
            Assert.IsTrue(_Play(victim, ActionType.BattleIdle), "被抓者進戰鬥應被接受");
            yield return _Drive(2);
            Assert.IsTrue(_Play(grabber, ActionType.GrabStart), "GrabStart 應在 BattleIdle 白名單內");

            var paired = false;
            yield return _DriveUntil(
                () => _Node(grabber) == ActionType.GrabIdleA && _Node(victim) == ActionType.GrabIdleB,
                5f, d => paired = d);
        ...
        public IEnumerator GrabConnectPairsAndAnchorsVictim()
        {
            ...
            Assert.IsFalse(_Play(victim, ActionType.BattleWalk), "被抓者 Move 目標應被拒收");
            Assert.IsFalse(_Play(victim, ActionType.GrabStart), "被抓者反抓應被拒收");
        ...
        [UnityTest]
        [Timeout(60000)]
        public IEnumerator GrabWhiffRecoversToBattleIdle()
        {
            var grabber = _Enter("Grabber", out _);
            yield return _Drive(2);
            Assert.IsTrue(_Play(grabber, ActionType.BattleIdle), "進戰鬥應被接受");
            yield return _Drive(2);
            Assert.IsTrue(_Play(grabber, ActionType.GrabStart), "空揮 GrabStart 應被接受");

            // 無目標:GrabStart 自然播完 → Next = BattleIdle,不得殘留 GrabIdleA
            var started = false;
            yield return _DriveUntil(() => _Node(grabber) == ActionType.GrabStart, 5f, d => started = d);
            Assert.IsTrue(started, "GrabStart 應開始");
            var recovered = false;
            yield return _DriveUntil(() => _Node(grabber) == ActionType.BattleIdle, 5f, d => recovered = d);
            Assert.IsTrue(recovered, $"空揮應自然回 BattleIdle(node={_Node(grabber)})");
        }
        ...
        public IEnumerator ThirdPartyHitOnGrabberDissolvesPair()
        {
            ...
            Assert.IsTrue(_Play(attacker, ActionType.BattleIdle), "第三方進戰鬥應被接受");
            yield return _Drive(2);
            Assert.IsTrue(_Play(attacker, ActionType.BattleAttack), "第三方攻擊應被接受");

            var dissolved = false;
            yield return _DriveUntil(
                () => _Node(grabber) == ActionType.BattleDamage && _Node(victim) == ActionType.BattleIdle,
                5f, d => dissolved = d);
            Assert.IsTrue(dissolved,
                $"打抓取者應解體:抓取者進硬直、被抓者獲釋(grabber={_Node(grabber)}, victim={_Node(victim)})");

            // 被抓者已自由:掙脫(只在被抓白名單)應被拒收,證明白名單已切換
            Assert.IsFalse(_Play(victim, ActionType.GrabBreakB), "獲釋後掙脫應不在白名單");
        }
        ...
        public IEnumerator LeaveGrabberFreesVictim()
        {
            ...
            var freed = false;
            yield return _DriveUntil(() => _Node(victim) == ActionType.BattleIdle, 5f, d => freed = d);
        ...
        public IEnumerator LeaveVictimFreesGrabber()
        {
            ...
            var freed = false;
            yield return _DriveUntil(() => _Node(grabber) == ActionType.BattleIdle, 5f, d => freed = d);
            Assert.IsTrue(freed, $"被抓者離場後抓取者應回 BattleIdle(node={_Node(grabber)})");
        }

        [UnityTest]
        [Timeout(60000)]
        public IEnumerator GrabWalkForwardsVictimAlongAnchor()
        {
            var grabber = _Enter("Grabber", out _);
            var victim = _Enter("Victim", out _);
            yield return _EstablishGrab(grabber, victim);

            // 拖行:抓取者往 +x 走,被抓者應鏡射到 GrabWalkB 並持續貼在新錨軸前方
            var dragDirection = new Vector2(1f, 0f);
            var start = grabber.Player.SamplePositionNow();
            Assert.IsTrue(_PlayDir(grabber, ActionType.GrabWalkA, dragDirection), "拖行走路應在 GrabIdleA 白名單內");

            var dragged = false;
            yield return _DriveUntil(
                () => _Node(victim) == ActionType.GrabWalkB &&
                      (grabber.Player.SamplePositionNow() - start).magnitude > 0.8f,
                5f, d => dragged = d);
            Assert.IsTrue(dragged, $"拖行應位移且被抓者進 GrabWalkB(victim={_Node(victim)})");

            // 拖行中被抓者的移動/停止都不可用(白名單只剩掙脫;Stop = Play(Next) 同樣被拒)
            Assert.IsFalse(_Play(victim, ActionType.GrabWalkB), "被抓者自行走路應被拒收");
            Assert.IsFalse(_Play(victim, ActionType.GrabIdleB), "被抓者 Stop(Play(Next))應被拒收");
            ...
            Assert.IsTrue(_Play(grabber, ActionType.GrabIdleA), "停止拖行應在 GrabWalkA 白名單內");
            var settled = false;
            yield return _DriveUntil(
                () => _Node(grabber) == ActionType.GrabIdleA && _Node(victim) == ActionType.GrabIdleB,
                5f, d => settled = d);
        ...
        [UnityTest]
        [Timeout(60000)]
        public IEnumerator GrabBreakFreesBothSides()
        {
            var grabber = _Enter("Grabber", out _);
            var victim = _Enter("Victim", out _);
            yield return _EstablishGrab(grabber, victim);

            Assert.IsTrue(_Play(victim, ActionType.GrabBreakB), "掙脫應在被抓白名單內");

            // 雙方進 Break 成對動作
            var breaking = false;
            yield return _DriveUntil(
                () => _Node(grabber) == ActionType.GrabBreakA && _Node(victim) == ActionType.GrabBreakB,
                5f, d => breaking = d);
            Assert.IsTrue(breaking, $"掙脫應帶動雙方 Break(grabber={_Node(grabber)}, victim={_Node(victim)})");

            // Break 播完各自回 BattleIdle;被抓者恢復自由(掙脫鍵已不在白名單)
            var freed = false;
            yield return _DriveUntil(
                () => _Node(grabber) == ActionType.BattleIdle && _Node(victim) == ActionType.BattleIdle,
                5f, d => freed = d);
            Assert.IsTrue(freed, $"Break 播完雙方應回 BattleIdle(grabber={_Node(grabber)}, victim={_Node(victim)})");
            Assert.IsFalse(_Play(victim, ActionType.GrabBreakB), "解體後掙脫應不在白名單");
        }

        [UnityTest]
        [Timeout(60000)]
        public IEnumerator GrabThrowLaunchesVictimByBakedMotion()
        {
            var grabber = _Enter("Grabber", out _);
            var victim = _Enter("Victim", out _);
            yield return _EstablishGrab(grabber, victim);
            ...
            Assert.IsTrue(_Play(grabber, ActionType.GrabThrowA), "丟投應在 GrabIdleA 白名單內");

            var thrown = false;
            yield return _DriveUntil(() => _Node(victim) == ActionType.GrabThrowB, 5f, d => thrown = d);
            Assert.IsTrue(thrown, $"被抓者應被強制進 GrabThrowB(node={_Node(victim)})");

            // 播完雙方回 BattleIdle;被抓者位移 = ThrowB 烘焙段(局部 -y 2.4m,基底 = 被抓者朝向(0,-1)→ 世界 +y)
            var done = false;
            yield return _DriveUntil(
                () => _Node(grabber) == ActionType.BattleIdle && _Node(victim) == ActionType.BattleIdle,
                5f, d => done = d);
            Assert.IsTrue(done, $"丟投播完雙方應回 BattleIdle(grabber={_Node(grabber)}, victim={_Node(victim)})");
            ...
            Assert.IsFalse(_Play(victim, ActionType.GrabBreakB), "丟投後掙脫應不在白名單");
        }
        ...
        public IEnumerator GrabAtkPlaysVictimReactionAndKeepsPair()
        {
            ...
            Assert.IsTrue(_Play(grabber, ActionType.GrabAtk1A), "抓取中補打應在 GrabIdleA 白名單內");

            // 補打命中 → Damage 路由到 GrabAtk1B(不經鏡射)
            var reacted = false;
            yield return _DriveUntil(() => _Node(victim) == ActionType.GrabAtk1B, 5f, d => reacted = d);
            Assert.IsTrue(reacted, $"被抓者應進受創反應 GrabAtk1B(node={_Node(victim)})");
            ...
            Assert.IsFalse(_Play(victim, ActionType.BattleWalk), "補打後被抓者移動仍應被拒收");
        }
    }
}
